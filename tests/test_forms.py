...
class TestSelectOption:
    ...
    @pytest.mark.asyncio
    async def test_select_dropdown_option(self, mcp_server, mock_browser_manager, mock_page, sample_element_map):
...
class TestCheckCheckbox:
    ...
    @pytest.mark.asyncio
    async def test_toggle_checkbox(self, mcp_server, mock_browser_manager, mock_page, sample_element_map):
        ...
        with patch('browsercontrol.tools.forms.browser', mock_browser_manager):
            with patch('browsercontrol.tools.forms.get_element_map', return_value=checkbox_map):
                mock_browser_manager.page = mock_page
                mock_browser_manager.screenshot_with_som.return_value = (b"screenshot", checkbox_map)
...
class TestUploadFile:
    ...
    @pytest.mark.asyncio
    async def test_upload_file_to_input(self, mcp_server, mock_browser_manager, mock_page, sample_element_map, tmp_path):
        ...
        with patch('browsercontrol.tools.forms.browser', mock_browser_manager):
            with patch('browsercontrol.tools.forms.get_element_map', return_value=file_input_map):
                mock_browser_manager.page = mock_page
                mock_browser_manager.screenshot_with_som.return_value = (b"screenshot", file_input_map)
